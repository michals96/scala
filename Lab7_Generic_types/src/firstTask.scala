...
Useful functions that you can use in parsing.
...
p:
  ...
  t: simple par with more text

 */

A processor mehtod `query` working as below:

val q = query( parsed, el => el match {

  case t: Text if t.tag == 't' || t.text.startsWith("P") => true
